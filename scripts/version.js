...
async function version() {
    ...
    if (pre.length) {
      if (!next)
        return Promise.resolve(
          semver.inc(current, "prerelease", false, pre[0])
        );
      ...
      const { prereleaseType } = await prompt({
        ...
        choices: [
          ...
          {
            title: `Patch ${dim(bumped.patch + "-" + prereleaseId + ".0")}`,
            value: "prepatch"
          },
          {
            title: `Minor ${dim(bumped.minor + "-" + prereleaseId + ".0")}`,
            value: "preminor"
          },
          {
            title: `Major ${dim(bumped.major + "-" + prereleaseId + ".0")}`,
            value: "premajor"
          }
        ],
        initial: 0
      ...
      return Promise.resolve(
        semver.inc(current, prereleaseType, false, prereleaseId)
      );
    }
  }
  ...
  if (type === "patch" || !pre[0]) {
    ...
    console.log(`
${yellow("▲")} Warning! Updating from ${green(current)} to ${green(
      bumped[type]
    )} skips ${next.map(cyan).join(", ")} prerelease${
      next.length > 1 ? "s" : ""
    }`);
